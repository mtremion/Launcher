    ...
    public void GetProfileImage(string pseudo)
    {
        ...
        try
        {
            ...
            if (_dataReader.Read())
            {
                 ...
                 if(isLoaded)
                 {
                     _goManager.m_goCanvasProfile.m_imgImgProfileCanvasProfile.sprite = Sprite.Create(sampleTexture, new Rect(0.0f, 0.0f, sampleTexture.width, sampleTexture.height), new Vector2(0.5f, 0.5f), 100.0f);
                 }
